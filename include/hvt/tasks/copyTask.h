...
/// CopyTask parameters.
struct HVT_API CopyTaskParams
{
    /// The name of the aov to copy.
    PXR_NS::TfToken aovName = PXR_NS::HdAovTokens->color;
};

/// A task for performing moving any intermediate results back to the color RenderBuffer for use in
/// other render passes.
class HVT_API CopyTask : public PXR_NS::HdxTask
{
public:
    /// Constructor
    /// \param delegate A task delegate to store input parameters in.
    /// \param uid The unique id for this task.
    CopyTask(PXR_NS::HdSceneDelegate* delegate, PXR_NS::SdfPath const& uid);
    ~CopyTask() override;
...
protected:
    ...
    /// Returns the path where is the copy shader text.
    static const PXR_NS::TfToken& _CopyShaderPath();
...
private:
    ...
    /// Copy to the currently bound framebuffer.
    void _ApplyCopy(PXR_NS::HgiTextureHandle const& aovTexture);
    ...
    CopyTaskParams _params;
};

/// VtValue requirements
/// @{
HVT_API std::ostream& operator<<(std::ostream& out, const CopyTaskParams& pv);
