...
//     or, remove the HdPageableValue design but make it more higher level and applicable to data
//     source only.
...
public:
    ...
    static Handle New(const PXR_NS::SdfPath& primPath,
        const std::unique_ptr<HdPageFileManager>& pageFileManager,
        const std::unique_ptr<HdMemoryMonitor>& memoryMonitor,
        DestructionCallback destructionCallback, HdBufferUsage usage = HdBufferUsage::Static);
...
private:
    HdPageableBlockDataSource(const PXR_NS::SdfPath& primPath,
        ...
        HdPageableBufferBase(
            primPath, 0, usage, pageFileManager, memoryMonitor, destructionCallback)
    {
    }
};
...
{
public:
    HdMemoryManager(std::filesystem::path pageFileDirectory, size_t sceneMemoryLimit,
...
PXR_NS::HdVectorDataSourceHandle CreateVector(const std::vector<PXR_NS::VtValue>& values,
    const PXR_NS::SdfPath& primPath, const std::shared_ptr<HdMemoryManager>& memoryManager);
