...
// Concept for objects that have a key
template <typename T, typename KeyType = PXR_NS::SdfPath>
concept Keyed = requires(T t)
{
    { t.Key() } -> std::convertible_to<KeyType>;
};

// Concept for objects that use SdfPath as key
// This is a backward-compatible alias for Keyed<T, SdfPath>
template <typename T>
concept Pathed = Keyed<T, PXR_NS::SdfPath>;
...
concept PagingStrategyLike =
    requires(T t, const HdPageableBufferCore& buffer, const HdPagingContext& context)
{
...
struct Keyed : std::false_type {};
template <typename T, typename KeyType>
struct Keyed<T, KeyType, std::void_t<decltype(std::declval<T>().Key())>>
    : std::is_convertible<decltype(std::declval<T>().Key()), KeyType> {};
template <typename T, typename KeyType = PXR_NS::SdfPath>
inline constexpr bool KeyedValue = Keyed<T, KeyType>::value;
...
struct _PagingStrategyLike<T,
    std::void_t<decltype(std::declval<T>()(
        std::declval<const HdPageableBufferCore&>(), std::declval<const HdPagingContext&>()))>>
{
    static constexpr bool value =
        ...
        std::declval<InputIterator>(), std::declval<const HdSelectionContext&>()))>>
{
    static constexpr bool value =
        std::is_convertible<decltype(std::declval<T>()(std::declval<InputIterator>(),
            std::declval<InputIterator>(), std::declval<const HdSelectionContext&>())),
            std::vector<std::shared_ptr<HdPageableBufferCore>>>::value;
};
