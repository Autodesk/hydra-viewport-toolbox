...
HdTaskSharedPtrVector FramePass::GetRenderTasks(RenderBufferBindings const& inputAOVs)
{
    ...
    else
    {
        ...
        if (!IsStormRenderDelegate(GetRenderIndex()) || params().enableOutline ||
            params().enableContourSilhouette)
        {
            renderOutputs.push_back(HdAovTokens->primId);
            renderOutputs.push_back(HdAovTokens->instanceId);
            if (params().enableContourSilhouette)
