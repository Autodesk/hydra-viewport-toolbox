...
// ADSK: For pending changes to OpenUSD from Autodesk.
    ...
    if (_passParams.clearBackgroundColor && _passParams.visualizeAOV == HdAovTokens->color)
    {
        _bufferManager->SetRenderOutputClearColor(
            HdAovTokens->color, VtValue(_passParams.backgroundColor));
    }

    if (!_passParams.clearBackgroundColor && _passParams.visualizeAOV == HdAovTokens->depth)
    {
