...
HdPagingDecision AgeBasedStrategy::operator()(
    const HdPageableBufferCore& buffer, const HdPagingContext& context) const
{
    HdPagingDecision decision;

    // Don't page dynamic buffers or mapped buffers, or buffers that are not over age
    if (buffer.Usage() == HdBufferUsage::Dynamic || context.bufferAge < context.ageLimit)
    {
        return decision;
    }
    ...
    auto state = static_cast<int>(buffer.GetBufferState());
...
HdPagingDecision PressureBasedStrategy::operator()(
    const HdPageableBufferCore& buffer, const HdPagingContext& context) const
{
    HdPagingDecision decision;

    // Don't page dynamic buffers or mapped buffers
    if (buffer.Usage() == HdBufferUsage::Dynamic)
    {
        return decision;
    }
    const bool highScenePressure =
        context.scenePressure > HdMemoryMonitor::HIGH_SCENE_PRESSURE_THRESHOLD;
    const bool highDevicePressure =
        context.rendererPressure >= HdMemoryMonitor::HIGH_RENDERER_PRESSURE_THRESHOLD;
...
HdPagingDecision ConservativeStrategy::operator()(
    const HdPageableBufferCore& buffer, const HdPagingContext& context) const
{
    HdPagingDecision decision;
    ...
    const bool veryHighScenePressure =
        context.scenePressure > HdMemoryMonitor::HIGH_SCENE_PRESSURE_THRESHOLD;
    const bool veryHighDevicePressure =
        context.rendererPressure > HdMemoryMonitor::HIGH_RENDERER_PRESSURE_THRESHOLD;
    ...
    else if (veryHighDevicePressure && veryOld)
    {
        ...
        decision.forceOperation = false;
        decision.action =
            HdPagingDecision::Action::SwapToSceneMemory; // Conservative: prefer memory over disk
    }

    return decision;
}

HdPagingDecision HybridStrategy::operator()(
    const HdPageableBufferCore& buffer, const HdPagingContext& context) const
{
    HdPagingDecision decision;
    ...
    const bool highDevicePressure =
        context.rendererPressure >= HdMemoryMonitor::HIGH_RENDERER_PRESSURE_THRESHOLD;
    const bool isOverAge = context.bufferAge >= context.ageLimit;

    // Use age-based strategy when pressure is moderate but buffer is old
    if (!highScenePressure && !highDevicePressure && isOverAge)
    {
