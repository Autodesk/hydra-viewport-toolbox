...
HdPageableValue::HdPageableValue(const SdfPath& path,
    ...
    : HdPageableBufferBase(path, estimatedSize, usage, pageFileManager, memoryMonitor, destructionCallback)
    , mSourceValue(data)
    , mDataType(dataType)
{
...
TfSpan<const std::byte> HdPageableValue::GetSceneMemorySpan() const noexcept {
    ...
    return TfSpan<const std::byte>(reinterpret_cast<const std::byte*>(buffer.data()), buffer.size());
}
...
HdPageableSampledDataSource::Handle
HdPageableSampledDataSource::New(const VtValue& value,
                               ...
                               DestructionCallback destructionCallback,
                               HdBufferUsage usage) {
    return Handle(new HdPageableSampledDataSource(value, primPath, attributeName, pageFileManager, memoryMonitor, destructionCallback, usage));
}

HdPageableSampledDataSource::Handle
HdPageableSampledDataSource::New(const std::map<Time, VtValue>& samples,
                               const SdfPath& primPath,
                               const TfToken& attributeName,
                               const std::unique_ptr<HdPageFileManager>& pageFileManager,
                               const std::unique_ptr<HdMemoryMonitor>& memoryMonitor,
                               DestructionCallback destructionCallback,
                               HdBufferUsage usage) {
    return Handle(new HdPageableSampledDataSource(samples, primPath, attributeName, pageFileManager, memoryMonitor, destructionCallback, usage));
}
...
HdPageableSampledDataSource::HdPageableSampledDataSource(const VtValue& value,
                                                     ...
                                                     DestructionCallback destructionCallback,
                                                     HdBufferUsage usage)
    : HdPageableBufferBase(primPath, HdPageableValue::EstimateMemoryUsage(value), usage, pageFileManager, memoryMonitor, destructionCallback)
    , mPrimPath(primPath)
    , mAttributeName(attributeName) {
    
    // Create single sample at default time
    auto bufferKey = GetBufferKey(0.0);
    auto buffer = std::make_shared<HdPageableValue>(SdfPath(bufferKey), Size(), usage, pageFileManager, memoryMonitor, [](const SdfPath&){}, value, attributeName);
...
VtValue HdPageableSampledDataSource::GetValue(Time shutterOffset) {
    ...
    auto it = std::lower_bound(mSamples.begin(), mSamples.end(), shutterOffset,
        [](const MemorySample& sample, Time time) {
...
HdMemoryManager::HdMemoryManager(std::filesystem::path pageFileDirectory,
    size_t sceneMemoryLimit, size_t rendererMemoryLimit)
    : mBufferManager({
        .pageFileDirectory = pageFileDirectory,
...
HdMemoryManager::GetOrCreateBuffer(const PXR_NS::SdfPath& primPath,
                                   const VtValue& data,
                                   const TfToken& dataType) {
    ...
    auto buffer = std::make_shared<HdPageableValue>(
        primPath, estimatedSize, HdBufferUsage::Static,
...
HdDataSourceBaseHandle CreateFromValue(const VtValue& value,
                                      const SdfPath& primPath,
                                      const TfToken& name,
    ...
    if (value.IsArrayValued()) {
        ...
        return HdPageableSampledDataSource::New(value, primPath, name, 
            memoryManager->GetPageFileManager(), 
            memoryManager->GetMemoryMonitor(),
            destructionCallback);
    } else {
        // Scalar values
        return HdPageableSampledDataSource::New(value, primPath, name,
...
PXR_NS::HdContainerDataSourceHandle CreateContainer(
    const std::map<TfToken, VtValue>& /*values*/,
    const SdfPath& /*primPath*/,
    const std::shared_ptr<HdMemoryManager>& /*memoryManager*/) {
    // TODO: ...
...
PXR_NS::HdSampledDataSourceHandle CreateTimeSampled(
    const std::map<HdSampledDataSource::Time, VtValue>& /*samples*/,
    const SdfPath& /*primPath*/,
    const TfToken& /*name*/,
    const std::shared_ptr<HdMemoryManager>& /*memoryManager*/) {
    // TODO: ...
