...
TEST(TestPageableBuffer, BasicPageableBuffer)
{
    ...
    hvt::DefaultBufferManager cache(desc);
    
    // Create some buffers
    constexpr size_t MB = 1024 * 1024;
    auto buffer1 = cache.CreateBuffer(PXR_NS::SdfPath("/VertexBuffer1"), 50 * MB); // 50MB
    auto buffer2 = cache.CreateBuffer(PXR_NS::SdfPath("/IndexBuffer1"), 30 * MB);  // 30MB
    auto buffer3 = cache.CreateBuffer(PXR_NS::SdfPath("/TextureBuffer1"), 100 * MB); // 100MB
    
    cache.GetMemoryMonitor()->PrintMemoryStats();
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        auto buffer = cache.CreateBuffer(PXR_NS::SdfPath(bufferName), 80 * MB);
        [[maybe_unused]] bool result = buffer->PageToRendererMemory();
...
TEST(TestPageableBuffer, MemoryPressure)
{
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        auto buffer = cache.CreateBuffer(PXR_NS::SdfPath(bufferName), 50 * MB);
        
        // Age some buffers
        if (i < 10) {
            buffer->UpdateFrameStamp(cache.GetCurrentFrame() - 25); // Make them old
        }
        ...
        float rendererPressure = cache.GetMemoryMonitor()->GetRendererMemoryPressure();
        float scenePressure = cache.GetMemoryMonitor()->GetSceneMemoryPressure();
...
TEST(TestPageableBuffer, AsyncOperations)
{
    ...
    for (int i = 0; i < 5; ++i) {
        std::cout << "  Work iteration " << (i + 1) << ", pending: " 
...
TEST(TestPageableBuffer, PagingStrategy)
{
    ...
    auto buffer1 = bufferManager.CreateBuffer(PXR_NS::SdfPath("/SmallBuffer"), 20 * MB, hvt::HdBufferUsage::Static);
    auto buffer2 = bufferManager.CreateBuffer(PXR_NS::SdfPath("/MediumBuffer"), 50 * MB, hvt::HdBufferUsage::Static);
    auto buffer3 = bufferManager.CreateBuffer(PXR_NS::SdfPath("/LargeBuffer"), 100 * MB, hvt::HdBufferUsage::Static);
    auto buffer4 = bufferManager.CreateBuffer(PXR_NS::SdfPath("/HugeBuffer"), 200 * MB, hvt::HdBufferUsage::Static);
    auto buffer5 = bufferManager.CreateBuffer(PXR_NS::SdfPath("/DynamicBuffer"), 75 * MB, hvt::HdBufferUsage::Dynamic);
    auto buffer6 = bufferManager.CreateBuffer(PXR_NS::SdfPath("/OldBuffer"), 30 * MB, hvt::HdBufferUsage::Static);
    auto buffer7 = bufferManager.CreateBuffer(PXR_NS::SdfPath("/VeryOldBuffer"), 40 * MB, hvt::HdBufferUsage::Static);
    ...
    std::cout << "...Synchronous FreeCrawl completed in " << duration.count() << "ms\n";
#endif
    ...
    // Show pending operations before FreeCrawl
    std::cout << "Pending async operations before FreeCrawl: " << bufferManager.GetPendingOperations() << "\n\n";
    ...
    std::cout << "...Async FreeCrawl completed in " << duration.count() << "ms\n";
#endif
    ...
    endTime = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
    std::cout << "...Aggressive async FreeCrawl completed in " << duration.count() << "ms\n";
#endif
