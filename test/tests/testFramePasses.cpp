...
// Note: The two frame passes are displayed in the left part on iOS. Refer to OGSMOD-7278.
#if defined(__ANDROID__) || TARGET_OS_IPHONE == 1
TEST(TestViewportToolbox, DISABLED_TestFramePasses_MultiViewportsFramePassClearBufferImprovements)
#else
TEST(TestViewportToolbox, TestFramePasses_MultiViewportsFramePassClearBufferImprovements)
#endif
{
    // The unit test mimics two viewports using frame passes.
    // The goal is to check that the depth buffer is cleared in the second frame pass.
    ...
    auto render = [&]()
    {
...

        {
            ...
            // Clear color and depth for the first 5 frames, then stop
