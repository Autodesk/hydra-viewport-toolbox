...
struct OutlineFixture
{
    ...
    OutlineFixture()
    {
        ...
        passDesc.uid         = SdfPath("/TestOutline");
...
// read back committed OutlineMaskTaskParams or OutlinePrimIdsTaskParams.
...
// Helper: reads back the committed OutlineMaskTaskParams from a TaskManager.
...
HVT_TEST(TestOutline, outline_styleEquality)
{
    hvt::OutlineStyle a;
    hvt::OutlineStyle b;
...
HVT_TEST(TestOutline, outline_styleDefaultValues)
{
...
HVT_TEST(TestOutline, outline_install)
{
    OutlineFixture f;
    hvt::Outline outline;
    ...
    ASSERT_FALSE(taskManager->HasTask(hvt::OutlinePrimIdsTask::GetToken("Base")));
    ASSERT_FALSE(taskManager->HasTask(hvt::OutlinePrimIdsTask::GetToken("Overlay")));
    ASSERT_FALSE(taskManager->HasTask(hvt::OutlinePrimIdsTask::GetToken("Default")));
    ASSERT_FALSE(taskManager->HasTask(hvt::OutlineMaskTask::GetToken()));
    ASSERT_FALSE(taskManager->HasTask(hvt::OutlineOverlayTask::GetToken()));

    outline.Install(*f.framePass);

    ASSERT_TRUE(taskManager->HasTask(hvt::OutlinePrimIdsTask::GetToken("Base")));
    ASSERT_TRUE(taskManager->HasTask(hvt::OutlinePrimIdsTask::GetToken("Overlay")));
    ASSERT_TRUE(taskManager->HasTask(hvt::OutlinePrimIdsTask::GetToken("Default")));
    ASSERT_TRUE(taskManager->HasTask(hvt::OutlineMaskTask::GetToken()));
    ASSERT_TRUE(taskManager->HasTask(hvt::OutlineOverlayTask::GetToken()));
}

/// Test: Verifies that calling Install() a second time is silently ignored
/// and does not duplicate tasks in the frame pass.
HVT_TEST(TestOutline, outline_installTwiceIsNoop)
{
    OutlineFixture f;
    hvt::Outline outline;
...
HVT_TEST(TestOutline, outline_taskOrderPrimIdsBeforeMaskBeforeOverlay)
{
...
HVT_TEST(TestOutline, outline_cacheFirstNonEmptyCallIsMiss)
{
    hvt::Outline outline;
...
HVT_TEST(TestOutline, outline_cacheHitOnIdenticalInputs)
{
    hvt::Outline outline;
...
HVT_TEST(TestOutline, outline_cacheMissOnChangedInputs)
{
    hvt::Outline outline;
...
HVT_TEST(TestOutline, outline_cacheStatsAccumulate)
{
    hvt::Outline outline;
...
HVT_TEST(TestOutline, outline_cacheMaxCollectionSize)
{
    hvt::Outline outline;
...
HVT_TEST(TestOutline, outline_cacheSetInputsDedupWithHoverPaths)
{
    OutlineFixture f;
    hvt::Outline outline;
    outline.Install(*f.framePass);
...
HVT_TEST(TestOutline, outline_setStyleDedup)
{
    OutlineFixture f;
    hvt::Outline outline;
    outline.Install(*f.framePass);
...
HVT_TEST(TestOutline, outline_maskTextureFallbackWhenDefaultDisabled)
{
    OutlineSceneFixture f;
    hvt::Outline outline;
    outline.Install(*f.framePass);
...
HVT_TEST(TestOutline, outline_maskTextureFallbackWhenOverlayEmpty)
{
    OutlineSceneFixture f;
    hvt::Outline outline;
    outline.Install(*f.framePass);
...
HVT_TEST(TestOutline, outline_excludePathsAppliedToDefaultCollection)
{
    OutlineSceneFixture f;
    hvt::Outline outline;
    outline.Install(*f.framePass);
...
HVT_TEST(TestOutline, DISABLED_outline_renderSelectedPath)
#else
HVT_TEST(TestOutline, outline_renderSelectedPath)
#endif
{
...
/// Test: Verifies that SetStyle() changes applied mid-render take effect and
/// produce the expected output per blur mode (None, Blur3x3, Blur5x5).
#if defined(__APPLE__)
HVT_TEST(TestOutline, DISABLED_outline_renderStyleChange)
#else
HVT_TEST(TestOutline, outline_renderStyleChange)
#endif
{
    if (GetParam() == HgiTokens->Vulkan)
    {
    ...
    auto render    = [&]()
    {
        ...
        // Advance the blur mode every three frames: None -> Blur3x3 -> Blur5x5.
        hvt::OutlineStyle style;
        style.selectedColor = GfVec4f(0.10f, 0.55f, 1.0f, 0.7f);
        style.blurMode      = kBlurSequence[(9 - frameCount) / 3];
        outline.SetStyle(style);
